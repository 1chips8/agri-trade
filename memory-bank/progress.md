...
## 2026-06-05：可交付演示说明收尾

### 已完成

- README 新增推荐演示流程，覆盖管理员、商家、普通用户和完整订单履约闭环。
- README 新增超时取消演示方法，说明如何临时调小超时分钟数和扫描间隔。
- README 新增交付亮点，突出订单状态原子流转、库存原子更新、超时取消、请求校验和前端拆包优化。

### 验证

- 检查 README 结构与现有功能状态一致，未再把订单超时取消描述为未实现。

### 遗留问题

- README 仍以文字演示脚本为主，尚未补充截图或录屏；当前对课程/现场演示交付已经够用。

## 2026-06-05：H2 演示数据初始化

### 已完成

- 新增 `DemoDataInitializer`，仅在 `local-h2` profile 且 `agri.demo-data.enabled=true` 时创建演示数据。
- 演示数据包含普通买家 `buyer`、已认证商家 `merchant`、`绿野鲜农演示店` 和 3 个已审核上架商品。
- README 补充 H2 演示数据启动方式、演示账号和商品说明。

### 验证

- `mvn -Dtest=DemoDataInitializerTest test`：1 个测试通过，覆盖演示账号、商家和商品初始化。

### 遗留问题

- 演示数据只覆盖用户、商家和商品，不预置订单；订单仍建议现场通过购物车下单生成，更适合展示交易链路。
